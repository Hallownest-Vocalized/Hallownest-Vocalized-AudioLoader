...
namespace HallownestVocalizedAudioLoader.MajorFeatures;
...
public static class RollCredits
{
    ...
    public static void Hook()
    {
        ...
        UnityEngine.SceneManagement.SceneManager.activeSceneChanged += (_, to) =>
        {
            if (to.name == CreditsSceneName)
            {
                ...
                aSource.spread = 0;
                HallownestVocalizedAudioLoaderMod.CoroutineHolder.StartCoroutine(CreditsRoll());
                if (isFromMenu == false)
                {
    ...
    private static IEnumerator CreditsRoll()
    {
        ...
        if (isFromMenu == false)
        {
            ...
            while (Thanks.activeSelf == true) {
                ...
                {
                    HallownestVocalizedAudioLoaderMod.CoroutineHolder.StartCoroutine(GameManager.instance.ReturnToMainMenu(GameManager.ReturnToMainMenuSaveModes.DontSave));
                }
    ...
    public static IEnumerator LoadCreditsFromMenu()
    {
        ...
        yield return HallownestVocalizedAudioLoaderMod.CoroutineHolder.StartCoroutine(UIManagerR.HideCurrentMenu());
